...
publish_discovery() 
{
    ...
    for c in $(seq 0 "$CPU_CORES"); do
        ...
        "{\"name\":\"CPU Core $c\",\"state_topic\":\"$MQTT_TOPIC\",\"value_template\":\"{{ value_json.cpu_core$c }}\",\"unit_of_measurement\":\"%\",\"unique_id\":\"${DEVICE_ID}_cpu_$c\",\"availability_topic\":\"$AVAIL_TOPIC\",\"device\":{\"identifiers\":[\"$DEVICE_ID\"],\"name\":\"$DEVICE_NAME\"}}"
    done
}
